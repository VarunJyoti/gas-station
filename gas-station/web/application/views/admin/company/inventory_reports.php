	...
	<?php } ?>
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>MIR(Monthly Inventory Reconciliation)<small></small></h2>
                    
                    <div class="clearfix"></div>
					
					<div style="float:right;">
     <form class="navbar-form"  action="<?php echo base_url('admin/company/inventory_reports');?>" method="GET">
  <tr>
...
<?php 
 ...
 <td>
  ...
  {
	  ?>
	 <option value="<?php echo $this->input->get("pid");?>"><?php echo ucfirst($this->gasolinereceived_model->getProductName($this->input->get("pid")));?></option>  
 <?php 
 }
 else{
  ?>
 ...
 <?php } ?>
  ...
  <td>
   <input type="submit" class="form-control" name="find" value="SEARCH">
	...
	<?php 
	if(array_filter($dateWiseRow) !=0)
	{
		//print_r($sale);
	?>
	<table class="footable table table-striped table-bordered table-white table-primary" width="100%"> 
<tr>
<td>Date</td><td>Open</td><td>Received</td><td>Sale</td><td>Balance</td><td>Booked Inv.</td><td>OverShort</td>

</tr>
	<?php 
	foreach ($dateWiseRow as $row)
    {
	if(count(array_filter($row['inventory'])) != 0)
	{
		//print_r($row['inventory']['total_balance']);
		?>
		<tr>
		...
		<?php 
		foreach ($row['inventory'] as $sales)
    {
   ...
   <?php
   $total_sale = $sale;
   $total_sales = ($total_sale *0.01)+130;
   $total_diff = $diff ;
   ?>
	
	
	<td colspan="2"><b><?php if($total_sales > $total_diff){?><span style="color:red;"><?php echo $this->daily_shift_model->NumberFormat($total_sales) ; ?></span><?php } else {?> <?php echo $this->daily_shift_model->NumberFormat($total_sales) ; ?><?php }?></b></td>
	<td><b>Total OverShort</b></td>
	
   
	<td><b><?php   echo $this->daily_shift_model->NumberFormat($diff) ; ?></b></td>
