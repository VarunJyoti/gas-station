...
class Daily_shift_model extends CI_Model{
 ...
 /*
 ##---------------------Daily Entry Update Codes starts here -------------------------------##
 */		
		...
		if($id)
		{
		    ...
		    {
			 $old_propane_sales = 0;
		        foreach(array_combine($pid,$old_sale) as $row1=> $old_sales)
			    {
			      $merged=$this->daily_shift_model->getMergedByPid($row1);
			
			      //$product_price  =	$this->getProductPrice($row1);
			      $product_old_price  = $this->getProductOldPrice($row1);
			      if($merged == 1)
				    {
			         $old_propane_sales += (($old_sales[row1])*($product_old_price));
		    ...
		    foreach(array_combine($pid,$sale) as $row1=> $sales)
			{
			 ...
			 $propane_product_old_price  =	$this->getProductOldPrice(64);
		        ...
		        if((!empty($checkNewprice)) && ($price_change_status == 1))
		        {
		            ...
		            {
			         $propane_sales += (($sales[row1])*($product_price));
		    ...
		    $amount_available = ($credit_cards+$credit_total+$drops_total+$payouts);
		    ...
		    foreach(array_combine($id,$pid) as $product_id=> $row)
			{
             ...
             $merged=$this->daily_shift_model->getMergedByPid($row);
 ...
 /*
 ##---------------------Daily Entry Update Codes ends here -------------------------------##
 */	

 /*
 ##---------------------Daily Entry Insert Codes starts here -------------------------------##
 */	
		else 
		{
		    ...
		    $amount_available = ($credit_cards+$credit_total+$drops+$payouts);
 ...
 /*
 ##---------------------Daily Entry Insert Codes ends here -------------------------------##
 */
...
// daily close code starts here
	...
	public function getProductId() 
	{
	 ...
	 public function getMergedByPid($pid) 
	{
	        $company_id = $this->getCompanyId();
			
			$cid =  $company_id['0']->c_id;
			$this->db->select('merged');
	...
	{  
	     ...
	     $created_date=  date("Y-m-d H:i:s", time());
		 $this->db->select('SUM(amount) as ccard_total', FALSE);
		 $this->db->where('shift',$shift_id);
		// $this->db->where('type','received');
		 $this->db->where("user_id", $user_id);
		 $this->db->where('daily_no	',$daily_no);
		 $this->db->where('c_id	',$cid1);
		 $this->db->order_by('created_date','DESC');
		 $query = $this->db->get('ccard_entry')->row_object();
  ...
  public function getSnapImage() 
	...
	{
		echo number_format(round((float)$number,2),2);
	}
